...
std::tuple<float, float> SHT20toDB::readSensor() const {
...
void SHT20toDB::writeSQL(std::tuple<float, float> values) {
...
void SHT20toDB::createDefaultConfig() const {
  ...
  std::ofstream configFile("SHT20toDB.cfg");
  configFile << text;
  configFile.close();
}
...
int SHT20toDB::readConfig() {
  std::cout << "Reading config..." << std::endl;

  std::ifstream configFile("SHT20toDB.cfg");
  std::string line;
  while (true) {
